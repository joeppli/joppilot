...
- **Spoof-safe vehicle channel** (SEC-01/04, ICD §1, audit-9) — the IoT per-thing mTLS topic policy already pins each vehicle to its own topic prefix; the cloud consumers now **bind the payload's `vehicleId` to that authenticated topic segment** (telemetry, heartbeat/link-loss, latch-EDR, logsync, ACKs, maneuver proposals) — a compromised vehicle cert can no longer forge another vehicle's telemetry or **evidence rows** (LEG-04). The cloud→vehicle **deadman ping is now signed** (Ed25519, like commands) and the edge watchdog only accepts a **verified, fresh, strictly-increasing** ping — a replayed/spoofed ping cannot keep a vehicle out of safe mode (RES-01). The **logsync ack is signed and boot-scoped**: the edge trims its offline event buffer only on an ack for its own `bootId`, so a stale ack from a previous boot can't drop unsynced evidence (RES-02 "lossless" hardened). Maneuver decisions are **validated against the proposal's own options** at both gates (ICD §6) — an option the ADS never offered is refused (`INVALID_OPTION` / edge NACK). `LIGHTS` (headlights/turn signals) is now **Mode-2-only** per ICD §4; hazards are a separate Mode 1 `HAZARD_LIGHTS` command valid in any operating zone.
...
> construction (the viewer role cannot publish). The IoT policy AWS requires on
> each Cognito identity is attached automatically on first sign-in — the console
> calls a Lambda behind the API Gateway authorizer, which derives the identity
> from the caller's own token and attaches it (idempotent, so repeat sign-ins
> are no-ops).
>
