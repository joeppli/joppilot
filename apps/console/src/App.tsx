...
/**
 * Cloud-mode gate: nothing of the console renders before a Cognito sign-in
 * (SEC-03). Local dev (cloudState 'off') has no identity provider and stays
 * open, exactly as before.
 */
function AuthGate({ children }: { children: ReactNode }) {
  const { cloudState } = useSession();
  if (cloudState === 'signed-out') return <Login />;
  return <>{children}</>;
}
