...
| DEV-14 | Admin zone API reclassifies to non-permit zone types (depot/private/permitted_test) without a permit artifact — only the public_test_permit path is permit+window gated | ICD §1: cantonal approvals/permits enter as controlled zone configuration, never a runtime toggle | M2-5 Device Shadow zone-config delivery: zone changes become configuration-pipeline only (validated against canton-approved zone definitions), free-form reclassification removed | command.controller.ts setZone comment / audit-5 |