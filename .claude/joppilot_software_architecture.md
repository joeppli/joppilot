...
| DEV-14 | Admin zone API accepts any permit/site-authorization id without validating it against a canton-approved **zone-definition catalog** (geometry + conditions). Tightened in M2-5 (authorization artifact + validity window, signed delivery, edge auto-revert past the window) and again in **M3-5** (permit `validFrom` + `speedLimitKmh` now enforced ON THE VEHICLE: zone inactive before the window opens, over-limit Mode 2 NACKed `PERMIT_SPEED_LIMIT`, sim speed clamped). What remains is the catalog itself | ICD §1: cantonal approvals enter as controlled zone configuration validated against approved zone definitions | AD-15 zone service: Aurora-backed catalog + geometry validation (with the DB-backed zone registry, DEV-11's closure trigger) | command.controller.ts setZone comment / audit-5, M2-5, M3-5 |
...
| DEV-22 | The ICD §4 Mode 1 mission/fleet commands **skip next stop · return to depot · send to charging station · route to specific location · acknowledge/clear warning** are absent from the contract command catalogue (no schema, no console control, no edge handling) | Full ICD §4 command catalogue (ICD §4 is itself marked for expansion) | The Dispatch service work (AD-15 sequencing + OP-9 ADS route/stop feed) — these commands presuppose stops/routes/charging targets that only exist with that feed; add them together, not as dead schema entries | contract index.ts union comment / audit M3-2 review |
| DEV-23 | The console-viewer IoT policy is attached to each operator's Cognito identity MANUALLY (one CloudShell `aws iot attach-policy` per identity) — AWS requires an IoT policy on authenticated Cognito identities in addition to the IAM role, and no automation performs the attach | Automated attach on first login (post-auth hook / backend endpoint) | Before real operator onboarding — at latest with the CloudFront/SPA phase (where the console gets its production login flow) | console-identity module comment / M3-4b |
| DEV-24 | The on-vehicle event log (RES-02/LEG-04, M3-6) is an in-memory bounded buffer — a kernel restart or power cycle loses unsynced events (sync is otherwise lossless: ack-confirmed, dedup-keyed on vehicleId+bootId+seq) | LEG-04: on-vehicle recording that survives power cycles (persistent storage on the VEA) | M4 real-vehicle work (persistent edge storage) — at latest with the August WORM/EDR evidence hardening | edge main.rs event-log comment / M3-6 |
| DEV-25 | The edge TTL gate (ICD §4: a stale command is not applied) compares the cloud `timestamp+ttlMs` against the edge clock with NO skew tolerance. In the sim cloud and edge share one machine clock, so this is exact; a real vehicle and cloud have independent clocks, so a valid command could be dropped (or a slightly stale one accepted) under clock drift | Time-synced command validity: NTP/PTP-synchronized clocks (already required by LEG-04 time-synced recording) + a skew tolerance tuned with the watchdog timers | M4-4 real-vehicle work — establish the clock-sync story and set the skew/tolerance with OP-7 (watchdog/timer tuning) during the pilot; do NOT loosen the gate before a real clock-sync design exists | edge main.rs TTL gate / audit-9 review |
| DEV-26 | `MARK_COMPLETED` carries only `stopId` — ICD §4 says the operator may "confirm OR correct type/weight record", so the correction payload (collection type / weight) is missing. The command is otherwise wired (in the union, inferMode→MODE1, edge accepts it) but has NO consumer, console control or recycling-data service yet | Full ICD §4 recycling payload with a consumer (the Recycling Data service, AD architecture §3 RCY) | Add the correction fields WITH the recycling-data work (same rule as DEV-22: no dead schema entries) — the fields need the RCY service + a console affordance to mean anything | contract index.ts MarkCompletedCommandSchema / audit-9 review |
| DEV-27 | Operator console SPA is served from **GitHub Pages** (static hosting, no CDN of ours, no WAF in front) | SPA on S3 + CloudFront (AD-20), WAF at the edge (AD-19) | The CloudFront + custom-domain phase — the same trigger DEV-1/2/15/16 already wait on; adopting Pages did not change the target state, only the interim host | `.github/workflows/deploy-site.yml` |
| DEV-28 | The deployed console offers **operator sign-in on a publicly reachable page** — any origin on the internet can reach the entry gate, the Hosted UI and (once CORS is opened) the API Gateway. MFA (SEC-03) is the only barrier in front of a real operator session. Compensating controls: the demo path issues **no** credential of any kind (no Cognito identity, no IoT policy, no fencing token — `EntryGate` + the demo guards in `SessionContext`), and every command still passes both gates | SEC-07 / LEG-02 [M]: operator access **restrictable** to the Swiss control-centre network | The Client VPN + IP-restriction work in the security-hardening block. Note both constraints require the *capability*, not that it is switched on today — so this is a deliberately widened surface, not a violation | `apps/console/src/mode/EntryGate.tsx` / public-demo work |
| DEV-29 | The published console reaches the cloud API through the **generated** `https://<api-id>.execute-api…` URL, and Vite inlines it at BUILD time. The API Gateway is a `destroy-billables` target, so every destroy+restore cycle mints a new api-id and silently invalidates the live bundle: the operator signs in, telemetry flows, and every command fails against a URL that no longer exists — which reads as an outage, not as a stale variable. Compensating control: the restore procedure documents the rebuild step in three places, and with no endpoint configured the console degrades honestly (`commandsDisabled` banner, view-only) rather than firing into the void | A **stable API custom domain** (`api.joeppli.ch`): ACM cert + `aws_apigatewayv2_domain_name` living OUTSIDE the destroy target set, with only the `aws_apigatewayv2_api_mapping` recreated alongside the API — the DNS name then survives every cycle and `VITE_AWS_API_ENDPOINT` becomes a constant. Putting the domain resource inside `module.apigw` would only move the problem: its regional target changes on recreate and breaks the registrar CNAME instead | The CloudFront + custom-domain phase — the same trigger DEV-1/2/15/16/27 wait on. Cheap to pull forward (ACM certs and HTTP API custom domains carry no standing cost); the one-time cost is two registrar DNS records, since `joeppli.ch` is not in Route 53 (infra has only the private IoT-endpoint zone) | `.github/workflows/deploy-site.yml` config note / `terraform-destroy-billables.yml` restore steps / README cost-control section |