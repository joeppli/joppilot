...
1. **Zone-mode matrisi ve aksiyon→mod sınıflandırması İKİ yerde yaşar** (Rust, TS import edemez): `packages/contract/src/index.ts` (`ZONE_MODE_MATRIX` / `inferMode`) ve `edge/sim/src/main.rs` (`mode_allowed_in_zone` / `diag_disruptive_allowed` / `infer_mode`). ICD §1 matrisi veya §4 komut kataloğu değişirse **ikisini birden** güncelleyin ve `services/command/test/smoke-edge.cjs` çalıştırın. Not: zarftaki `mode` alanı her iki kapıda da **aksiyondan türetilir** — Gate 1 istemciden mod almaz, araç (Gate 2) beyan edilen modu türettiğiyle karşılaştırıp uyuşmazlıkta `MODE_MISMATCH` NACK'ler (ICD §1: bulut beyanı tek başına güvenilir girdi değildir). Kalıcı çözüm (M3-1 hedefi): matrisi diğer şemalar gibi üretilmiş JSON olarak yayınlamak ve edge'in açılışta okuması.
...
3. **`CommandEnvelope.correlationId` ZORUNLU (M2-4'te kapatıldı)** — EDR denetim zinciri (SEC-06) kayıtları bu id ile ilişkilendirir. Gate 1 her zarfta üretir (console göndermez), her EDR satırı `correlationId` kolonunda taşır, araç correlationId'siz zarfı `SCHEMA_INVALID` ile NACK'ler. Manevra akışında `proposalId` correlation id olarak kullanılır — öneri → karar → sonuç zinciri (ICD §10) tek id altında okunur.
